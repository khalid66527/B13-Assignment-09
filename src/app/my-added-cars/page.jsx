...
const AddedCarPage = async () => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/myCars/${user.id}`, {
        headers: {
    ...
    return (
        <div className="w-11/12 mx-auto">
            <h1 className="text-5xl font-bold mb-8 text-white text-center my-4">My Added Cars</h1>
            <p className="text-center text-gray-300 text-sm md:text-base mb-8">
                View all your added cars, manage listings, and track your rental history in one place.
            </p>
            <div>
                {
                    addeds.length === 0 ? (
                        <div className="w-full flex items-center justify-center py-16">
                             <div className="bg-white/80 backdrop-blur-md border border-gray-200 rounded-2xl px-10 py-8 shadow-md text-center hover:shadow-xl transition-all duration-300">
                                 ...
                                 >
                                     Add Car Now <FaArrowRightFromBracket />
                                 </Link>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-2xl font-semibold text-green-600">
                                             ...
                                             <p className="text-lg text-gray-900" suppressHydrationWarning>
                                                 <span className="font-bold">Added Date:</span> {new Date(added.addedDate || added.createdAt || Date.now()).toLocaleDateString("en-US", {
                                                     weekday: "short",
                                                     day: "numeric",
                                                     month: "long",
                                                     year: "numeric"
                                             ...
                                             <p className=" text-gray-900 text-lg"> <span className="font-bold">Added by :</span>{added.userName || user?.name}</p>
                                         </div>
                                     </div>

                                     {/* RIGHT SIDE BUTTONS */}
                                     <div className="flex flex-col gap-4">
                                         <EditCarModal carDetails={added} />
                                         <DeleteAdded carDetails={added} />
                                     </div>
                                </div>
                            </div>
                        ))
